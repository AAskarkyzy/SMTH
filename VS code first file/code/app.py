...
#создает соед-е, получение обьекта курсора(for sql запросов)
sqlite = sqlite3.connect("ovce.db").cursor()
...
@app.route("/datasets")
def datasets():
    #связано с формой которая в другом файле с формами
    sheep_form = SheepForm()
    #валидате анау-мнау определяет была ли форма отправлена методом POST
    if sheep_form.validate_on_submit():
        #извлекает введенное имя из формы
        sheep_name = sheep_form.sheepname.data
        #скл запрос для поиска подходящей овцы
        sheeps = sqlite3.execute("SELECT * FROM sheep WHERE name = ?", (sheep_name, ))
        #просто обновляем содерживое стр
